...
class TripletMiningMultipleModel(ImageCleanModel):
    def __init__(self, opt):
        super(TripletMiningMultipleModel, self).__init__(opt)
    ...
    def compute_loss(self, gt, preds, embedding=None, labels = None, return_loss_all=False):
        ...
        if embedding is not None and labels is not None:
            ...
            if embedding.shape[1] % num_cXgroup != 0:
                raise ValueError("Embedding dimension must be divisible by num_cXgroup but got {} and {}".format(embedding.shape[1], num_cXgroup))

            for i in range(embedding.shape[1] // num_cXgroup):
                temp_embedding = embedding[:, i*num_cXgroup:(i+1)*num_cXgroup].reshape(embedding.shape[0], -1)
                indices_tuple = self.miner(temp_embedding, labels)
                loss_dict[f"l_triplet_g{i}"] = self.TripletLoss(temp_embedding, labels, indices_tuple) * self.l_tr
                loss_all += loss_dict[f"l_triplet_g{i}"]
