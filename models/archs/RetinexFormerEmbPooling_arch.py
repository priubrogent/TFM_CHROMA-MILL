...
class Denoiser_emb_pooling(nn.Module):
    def __init__(self, in_dim=3, out_dim=3, dim=31, level=2, num_blocks=[2, 4, 4]):
        super(Denoiser_emb_pooling, self).__init__()
        self.dim = dim
        self.level = level
    ...
    def forward(self, x, illu_fea, I): # x: input image    illu_fea: output of illumination estimator I: real intensity
        """
        ...
        self.embedding_done = self.global_pooling(fea).clone()
        ...
        return out, fea[:,0:1,:,:].clone()

    def getEmbedding(self):
        return self.embedding_done

class RetinexFormer_Single_Stage_Emb_Pooling(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, level=2, num_blocks=[1, 1, 1], use_prior=True, I = 0, use_I = False, use_he = False):
        super(RetinexFormer_Single_Stage_Emb_Pooling, self).__init__()
        self.estimator = Illumination_Estimator(n_feat, use_prior = use_prior, I = I, use_I = use_I, use_he = use_he)
        self.denoiser = Denoiser_emb_pooling(in_dim=in_channels,out_dim=out_channels,dim=n_feat,level=level,num_blocks=num_blocks)  #### 将 Denoiser 改为 img2img
    ...
    def forward(self, img):
        ...
        illu_fea, illu_map = self.estimator(img, I = I)
        input_img = img * illu_map + img
        output_img, illu_pred = self.denoiser(input_img, illu_fea, I = I)
...
class RetinexFormerEmbPooling(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1,1,1], use_prior=True, I = 0, use_I = False, use_he = False):
        super(RetinexFormerEmbPooling, self).__init__()
        self.stage = stage

        modules_body = [RetinexFormer_Single_Stage_Emb_Pooling(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, level=2, num_blocks=num_blocks, use_prior=use_prior, I = I, use_I = use_I, use_he = use_he)
