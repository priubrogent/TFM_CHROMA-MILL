...
def calculate_psnr_batch_vectorized(img1_batch, img2_batch, crop_border, input_order='HWC', test_y_channel=False, type=None):
    """Calculate PSNR (Peak Signal-to-Noise Ratio) for a batch of images using vectorized operations.

    Args:
        img1_batch (ndarray/tensor): Batch of images with range [0, 255]/[0, 1].
        img2_batch (ndarray/tensor): Batch of images with range [0, 255]/[0, 1].
        crop_border (int): Cropped pixels in each edge of an image.
        input_order (str): Whether the input order is 'HWC' or 'CHW'. Default: 'HWC'.
        test_y_channel (bool): Test on Y channel of YCbCr. Default: False.

    Returns:
        numpy.ndarray: PSNR values for each image in the batch.
    """
    # Convert tensors to numpy arrays if necessary
    if isinstance(img1_batch, torch.Tensor):
        img1_batch = img1_batch.detach().cpu().numpy()
    if isinstance(img2_batch, torch.Tensor):
        img2_batch = img2_batch.detach().cpu().numpy()

    # esta entre 0 i 1
    # print("max img1_batch", img1_batch.max())
    # print("max img2_batch", img2_batch.max())

    # print("min img1_batch", img1_batch.min())
    # print("min img2_batch", img2_batch.min())

    # Convert to float64
    img1_batch = img1_batch.astype(np.float64)
    img2_batch = img2_batch.astype(np.float64)

    # Compute MSE for the entire batch
    mse_batch = np.mean((img1_batch - img2_batch) ** 2, axis=(1, 2, 3))

    # Handle the case where MSE is zero
    mse_batch[mse_batch == 0] = np.inf

    # Calculate PSNR
    max_value = 1. if img1_batch.max() <= 1 else 255.
    psnr_batch = 20. * np.log10(max_value / np.sqrt(mse_batch))

    # print('..psnr_batch', psnr_batch)
    return float(psnr_batch.mean().squeeze())
